...
El cierre técnico y metodológico de este ciclo marca la culminación exitosa de los objetivos planteados para el Sprint 2, consolidando la transición de la plataforma Agrosafe desde un modelo conceptual hacia un ecosistema de software interconectado y completamente funcional. A través de una dinámica de liderazgo compartido y una rigurosa gobernanza técnica en GitHub, el equipo materializó el desarrollo y despliegue de las segundas versiones (V2) de la Landing Page y de la Aplicación Web, optimizando la experiencia de usuario a partir del feedback de las entrevistas de validación y las evaluaciones heurísticas. Esta entrega cristaliza la arquitectura de software diseñada en las fases previas, demostrando una construcción colaborativa donde cada miembro aportó desde su especialidad técnica para asegurar que el incremento del producto responda con rigurosidad a los requisitos de control de fertirrigación y seguridad planteados por la startup SATECHO.

El núcleo del valor de este sprint radica en la integración exitosa de todo el ecosistema multi-tier, logrando por primera vez la comunicación bidireccional entre el hardware de campo, la capa de borde y la infraestructura en la nube. Este hito se alcanzó mediante la implementación de las primeras versiones funcionales del firmware del dispositivo embebido (Embedded ESP32) para la captura automatizada de telemetría de suelo, la API de Borde (Edge API) operando localmente de manera resiliente, la Aplicación Móvil (Mobile App) desarrollada en Flutter con soporte offline-first, y el Servicio de Backend (Backend Service) en la nube de Azure encargado de la orquestación, las reglas de alerta y el despacho de comandos automáticos hacia las electroválvulas. El cumplimiento de las metas del sprint queda plenamente validado al contrastar de forma cruzada las evidencias analíticas de desarrollo, la suite de pruebas unitarias e integrales, los contratos de servicios documentados y los artefactos de despliegue continuo en producción, garantizando una trazabilidad sin fisuras que respalda científicamente las decisiones de ingeniería del equipo y proporciona una plataforma robusta y transparente para el soporte de decisiones agrícolas.

**TB2**

El cierre metodológico y técnico del proyecto Agrosafe consolida la materialización de un ecosistema multi-tier e interconectado de alta resiliencia, diseñado para mitigar de forma científica las brechas de ineficiencia hídrica, bloqueo salino y vulnerabilidad patrimonial en la agricultura familiar de pequeñas y medianas parcelas en el Perú. Al integrar en una solución unificada el firmware embebido del microcontrolador ESP32 para la ingesta robusta de telemetría de suelo, la inteligencia local de la API de Borde en Flask, la lógica distribuida de los Servicios de Backend en Azure y las interfaces optimizadas de la Aplicación Web, la Aplicación Móvil con enfoque offline-first y la Landing Page, se concluye que el sistema logra con éxito transformar la experiencia puramente empírica del campo en una gestión predictiva basada en datos en tiempo real. El uso de una interfaz simplificada tipo "semáforo" para el agricultor y un panel analítico multi-cliente con control de umbrales para el ingeniero agrónomo valida que la tecnología AgTech puede ser adoptada sin fricciones cognitivas tanto por perfiles técnicos como no técnicos, sentando un precedente inmutable de gobernanza automatizada de código y hardware que respalda la escalabilidad operativa de la startup SATECHO.

Como directrices estratégicas y recomendaciones para futuros investigadores o desarrolladores que busquen iterar y expandir el valor de este producto de software, se aconseja priorizar la evolución de la inteligencia del sistema mediante la migración del motor analítico local hacia modelos predictivos avanzados de Machine Learning capaces de anticipar plagas y predecir el rendimiento preciso de la cosecha cruzando la data de sensores con APIs meteorológicas globales. En el ámbito de la infraestructura embebida y de borde, se recomienda transicionar el protocolo de comunicación de los nodos sensores hacia tecnologías de redes de bajo consumo y largo alcance como LoRaWAN, lo que independizará la solución de la cobertura Wi-Fi/celular rural y optimizará drásticamente la vida útil de las baterías mediante el uso de algoritmos avanzados de suspensión (deep sleep) y hardware con resistencia química a la corrosión salina. Finalmente, en la capa de aplicaciones, se sugiere robustecer el canal de notificaciones multicanal con flujos conversacionales interactivos en WhatsApp que permitan la apertura y cierre de las electroválvulas con un solo mensaje de texto sin necesidad de abrir la aplicación nativa, manteniendo siempre la suite de pruebas unitarias y de componentes por encima del ochenta por ciento de cobertura como garantía innegociable de estabilidad y mantenibilidad ante cualquier nuevo incremento de código.
